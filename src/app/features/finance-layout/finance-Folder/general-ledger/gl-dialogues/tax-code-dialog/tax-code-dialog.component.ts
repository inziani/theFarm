...
import { GLMasterDataTaxCodeFormGroup } from '@app/features/finance-layout/finance-Folder/finance-models/fi-form-models/gl-master-data-model';
    ...
    this._financeService.deleteTaxCode(this.taxCodeData.id).subscribe({
      next: (taxCodeDeleted) =>
        this._matDialog.open(DeleteDialogComponent, {
          data: this.taxCodeData.taxCodeDescription,
        }),
      error: (err) => (this.errorMessage = err),
