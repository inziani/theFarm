...
export class ChartOfAccountsDialogComponent implements OnInit {
  ...
  public onCreateChartOfAccounts() {
    ...
    this.financeService.createChartOfAccountsMasterData(
      ...
      next: (chartOfAccountsCreated) => this.dialog.open(ChangesSavedDialogComponent),
      error: (error) => this.dialog.open(ErrorHandlingDialogComponent),
  ...
  public onEditChartOfAccounts() {
    ...
    this.financeService.editSingleChartOfAccountsMasterData(
      ...
      subscribe({
        next: (chartOfAccountsEdited) => this.dialog.open(ChangesSavedDialogComponent),
        error: (error) => this.dialog.open(ErrorHandlingDialogComponent),
        complete: () => console.info('complete')
      });
    this.formGroup.reset();
  ...
  public onDeleteChartOfAccounts() {
    this.financeService.
      ...
      subscribe({
        next: (chartOfAccountsDeleted) => this.dialog.open(DeleteDialogComponent),
        error: (error) => this.dialog.open(ErrorHandlingDialogComponent),
        complete: () => console.info('complete')
      });
  }
