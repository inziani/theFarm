...
export class ActivityFormGroup extends FormGroup {
  constructor() {
    super({
      id: new ActivityFormControl('Id', 'id', '', Validators.compose([])),
  ...
  get ActivityFormControl(): ActivityFormControl[] {
    return Object.keys(this.controls).map(
      (k) => this.controls[k] as ActivityFormControl
    );
  }

  getTitleNameValidationMessages(title: string): string[] {
    return (
      this.controls['title'] as ActivityFormControl
    ).getValidationMessages();
  }

  getDescriptionValidationMessages(description: string): string[] {
    return (
      this.controls['description'] as ActivityFormControl
    ).getValidationMessages();
  }

  getStatusValidationMessages(status: string): string[] {
    return (
      this.controls['status'] as ActivityFormControl
    ).getValidationMessages();
  }

  getActivityCategoryValidationMessages(activity_category: string): string[] {
    return (
      this.controls['activity_category'] as ActivityFormControl
    ).getValidationMessages();
  }
  ...
  getFormValidationMessages(): string[] {
    ...
    Object.values(this.controls).forEach((c) =>
      messages.push(...(c as ActivityFormControl).getValidationMessages())
    );
    return messages;
  }
}
